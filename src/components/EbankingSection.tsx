...
export function EbankingSection({ rows, now }: { rows: RateRowData[]; now: number }) {
  if (rows.length === 0) return null

  return (
    <div className="section section--ebanking">
      <div className="section__title mono-label">
        eBanking <span className="section__title-note">· solo homebanking, no comparable con efectivo</span>
      </div>
      {rows.map((row) => (
        <RateRow key={row.key} row={row} now={now} showKind={false} />
      ))}
    </div>
  )
}
